...
TEST_CASE("post() schedules a coroutine and run() executes it", "[io_context]") {
    ...
    auto task_fn = [&]() -> Task<> { ran = true; ctx.stop(); co_return; };
    auto task = task_fn();
...
TEST_CASE("stop() terminates run() even with tasks remaining", "[io_context]") {
    ...
    auto stopper_fn = [&]() -> Task<> { ++count; ctx.stop(); co_return; };
    auto never_fn   = [&]() -> Task<> { ++count; co_return; };
...
TEST_CASE("current() returns running context inside run()", "[io_context]") {
    ...
    auto task_fn = [&]() -> Task<> { seen = IoContext::current(); ctx.stop(); co_return; };
    auto task = task_fn();
    ctx.post(task.native_handle());
    ctx.run();
...
TEST_CASE("post() from another thread wakes the event loop", "[io_context]") {
    ...
    auto waiter_fn  = [&]() -> Task<> { co_return; };
    auto stopper_fn = [&]() -> Task<> { task_ran = true; ctx.stop(); co_return; };
...
TEST_CASE("run() logs start and stop to clog without throwing", "[io_context]") {
    auto ctx = make_io_context();
    auto task_fn = [&]() -> Task<> { ctx.stop(); co_return; };
    auto task = task_fn();
    ctx.post(task.native_handle());
