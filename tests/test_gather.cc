...
static Task<int> immediate(int val) {
  co_return val;
}
static Task<> immediate_void() {
  co_return;
}
...
TEST_CASE("gather mixed void and value tasks", "[gather]") {
  auto [mono, val] = test::run_task<std::tuple<std::monostate, int>>(gather(immediate_void(), immediate(99)));
  CHECK(val == 99);
}
...
TEST_CASE("gather tasks run concurrently (timing)", "[gather]") {
  ...
  auto fn = [&]() -> Task<> {
    co_await gather(sleep(50ms), sleep(50ms), sleep(50ms));
  };
  test::run_task<void>(fn());
